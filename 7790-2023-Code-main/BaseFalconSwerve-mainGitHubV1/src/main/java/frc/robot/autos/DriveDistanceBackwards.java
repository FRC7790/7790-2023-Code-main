...
public class DriveDistanceBackwards extends SequentialCommandGroup
{
    public DriveDistanceBackwards(final Swerve s_Swerve, final double distance, final float rotation) {
        super(new Command[0]);
        final TrajectoryConfig config = new TrajectoryConfig(4.0, 8.0).setKinematics(Constants.Swerve.swerveKinematics);
        config.setReversed(true);
        final double dist = distance;
        final Trajectory exampleTrajectory = TrajectoryGenerator.generateTrajectory(new Pose2d(0.0, 0.0, new Rotation2d(0.0)), List.of(), new Pose2d(-dist, 0.0, new Rotation2d(0.0)), config);
